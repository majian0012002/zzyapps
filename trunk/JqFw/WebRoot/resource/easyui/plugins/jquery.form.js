...
$.extend($.fn.form.methods,{
	 ...
	 * @return {}
	 */
	'ajax': function(jq, param){
		return jq.each(function(){
			if (param.onSubmit){
				if (param.onSubmit.call(this) == false) {
					return;
				}
			}
			$.query(param['url'],param['data']||$(this).form('getValue'), param['success'], param['error']);
	 ...
	 * 获取Form所有组件的数据
	 */
	findField: function(jq, name){
		if(name.indexOf('.') != -1){
			var els = jq.form('getAllFields');
		...
		return jq.find('*[name='+name+']');
	},
	
	/**
	 * 获取值，到时根据name拼接成json对象
	 * 如：组件form有name:user.name,user.id,dep.id，
	 * 获取值后会拼接成{user:{name:11,id:22}, dep:{id:33}}
	 * 
	 * 以便以后跟后台交互
	 */
	getValue: function(jq, isJson){
		var valArr = jq.form('getValueForArray');
		if(isJson == true){
			var valJson = {};
			for (var index = 0; index < valArr.length; index++) {
				var param = valArr[index];
				valJson[param['name']] = param['value'];
			}
			return valJson;
		}else{
			return jQuery.param(valArr);
		}
	},
