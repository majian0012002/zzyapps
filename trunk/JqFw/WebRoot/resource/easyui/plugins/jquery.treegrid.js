...
(function($) {
	...
	function _14(_15, _16) {
		...
		if (_17.rownumbers
				|| (_17.frozenColumns && _17.frozenColumns.length > 0)) {
			if (_16) {
				_1c(_16);
				_1b.find("tr[node-id=" + _16 + "]").next("tr.treegrid-tr-tree")
	...
	function _28(_29) {
		...
		_2c.find("span.tree-hit").unbind(".treegrid").bind("click.treegrid",
				function() {
		...
		_2c.find("tr[node-id]").unbind(".treegrid").bind("mouseenter.treegrid",
				function() {
					var id = $(this).attr("node-id");
					_2c.find("tr[node-id=" + id + "]")
...
//					if(!_2a[uuid]){
//						alert(uuid);
//						initDD($(this).parent().children());
//						_2a[uuid] = true;
//					}
//					
//					function initDD(target){
//						alert(target.length)
...
//					}
					...
					_2a.onSelect.call(_29, _46(_29, id));
		...
		_2c.find("div.datagrid-cell-check input[type=checkbox]")
		...
		_2d.find("input[type=checkbox]").unbind().bind("click.treegrid",
				function() {
					if (_2a.singleSelect) {
						return false;
					}
	...
	function _30(_31, _32) {
		...
		var tr2 = _36.children("div.datagrid-body").find("tr[node-id=" + _32
				+ "]");
		var _37 = $(_31).datagrid("getColumnFields", true).length
				+ (_33.rownumbers ? 1 : 0);
		...
		function _39(tr, _3a) {
			$("<tr class=\"treegrid-tr-tree\">"
					+ "<td style=\"border:0px\" colspan=\"" + _3a + "\">"
	 ...
	 * 处理数据，渲染组件
	 */
	function _3b(_3c, _3d, _3e/*渲染值*/, _3f) {
		...
		var _45 = _46(_3c, _3d);
		if (_45) {
			var _47 = _43.children("div.datagrid-body").find("tr[node-id="
					+ _3d + "]");
			var _48 = _44.children("div.datagrid-body").find("tr[node-id="
					+ _3d + "]");
			var cc1 = _47.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			var cc2 = _48.next("tr.treegrid-tr-tree").children("td")
					.children("div");
		} else {
			var cc1 = _43.children("div.datagrid-body")
		...
		if (_40.showFooter) {
			_40.view.renderFooter.call(_40.view, _3c, _43
							.find("div.datagrid-footer-inner"), true);
			_40.view.renderFooter.call(_40.view, _3c, _44
		...
		//zzy+ 对于expand:true的行进行扩展 
		if(_3e['rows']){
			var rows = _3e['rows'];
			var idField = _40['idField'];
			for (var index = 0; index < rows.length; index++) {
				var row = rows[index];
				if(row['expand'] == true){
					$(_3c).treegrid('expand', row[idField]);					
				}
			}
		}
	};
...
//			_54.queryParams = _51;
//		}
		_54.queryParams = _54.queryParams || {};
		var param = $.isFunction(_54.queryParams) ? _54.queryParams() : _54.queryParams;
//		var _56 = $.extend({}, _54.queryParams);
		var _56 = $.extend( _51||{}, param||{});
		
		//  如果第一次加载,并且有root，则直接渲染数据
		if(_51== null && $.isObject(_54['root'])){
			_3b(_4f, _50, {rows:[_54['root']]}, _52);
			return;
		}
		...
		// zzy+ 用通用的ajax组件$.query
		$.query(_54.list, _56, function(_58){
			_58 = $.decode(_58); // zzy+ 是个BUG
			if(_56 && $.isArray(_58['rows'])){
				var rows = _58['rows'];
				for (var index = 0; index < rows.length; index++) {
					var row = rows[index];
					// 遍历加载的数据
					_54.eachLoadData.call(_4f, row, index);
					
					if(!row['iconCls']){
						row['iconCls'] = 'icon-tree-folder';
					}
					if(!row['parentId']){
						row['parentId'] = _56[_54['idField']];
					}
					if(!row['state']){
						row['state'] = 'closed';
					}
...
//				});
	 ...
	 * 获取父节点的id
	 */
//	function processRow(jq, row){
//		// 如果没有配置，则默认使用_parentId属性
//		if(!$.isEmpty(row['_parentId'])){
//			return row
//		};
//		
//		var parentField = $.data(jq, "treegrid").options['parentField'];
//		
//		// 如果字段是对象形式，则获取
...
//		
//		
//		if($.isObject(row[parentField])){
//			row['_parentId'] = $.beanVal(row[parentField], ); 
//		}else{
//			row['_parentId'] = row[parentField]; 
//		}
//		
...
//	}
	...
	function _73(_74, _75) {
		...
		var _78 = _77.find("div.datagrid-body tr[node-id=" + _75 + "]")
				.children("td[field=" + _76.treeField + "]");
	...
	function _a6(_a7, _a8) {
		...
		if (_a8.parent) {
			...
			var _ab = tr.children("td[field=" + _a9.treeField + "]")
	...
	function _ad(_ae, _af) {
		...
		if (_b2) {
			if (_b2.children.length == 0) {
				...
				var _b3 = tr.children("td[field=" + _b0.treeField + "]")
						.children("div.datagrid-cell");
				_b3.find(".tree-icon").removeClass("tree-folder")
						.addClass("tree-file");
	...
	$.fn.treegrid = function(_b5, _b6) {
		...
		return this.each(function() {
					...
					var opts=null;
					if(opts=$.data(this, "treegrid").options){
						if(isFirstRender){
							_1(this);
						}
	...
	$.fn.treegrid.methods = {
		...
		reload : function(jq, id) {
			return jq.each(function() {
						if (!$.isEmpty(id)) {
							...
							var _bd = $(this).datagrid("getPanel")
		...
		reloadFooter : function(jq, _be) {
			return jq.each(function() {
						...
						var _c0 = $(this).datagrid("getPanel")
						...
						if (_bf.showFooter) {
							_bf.view.renderFooter.call(_bf.view, this, _c1
											.find("div.datagrid-footer-inner"),
									true);
							_bf.view.renderFooter.call(_bf.view, this, _c2
											.find("div.datagrid-footer-inner"),
									false);
	...
	$.fn.treegrid.parseOptions = function(_c6) {
		var t = $(_c6);
		var cfg = $.extend({}, $.fn.datagrid.parseOptions(_c6), {
					treeField : t.attr("treeField"),
					animate : (t.attr("animate")
							? t.attr("animate") == "true"
							: undefined),
					root: t.attr("root")? $.decode(t.attr("root")) : null, //zzy+
	...
	var _c7 = $.extend({}, $.fn.datagrid.defaults.view, {
		render : function(_c8, _c9, _ca) {
			...
			$(_c9).append(ctEl);
//			alert($.uuidVal());
//			alert(ctEl.find('tr[node-id*=][uuid*=]').length);
			
//			var indicator = $('<div class="indicator">>></div>').appendTo('body');
//			ctEl.find('tr[node-id*=][uuid*=]').draggable({
//				revert:true,
//				deltaX:0,
//				deltaY:0
//			}).droppable({
...
//			});
			...
			function _cf(_d0, _d1, _d2) {
				...
				for (var i = 0; i < _d2.length; i++) {
					...
					_d3.push("<tr uuid="+uuid+" node-id=" + row[_cb.idField] + " " + _d5
							+ ">");
					_d3 = _d3.concat(_cd.renderRow.call(_cd, _c8, _cc, _d0,
							_d1, row));
					_d3.push("</tr>");
		...
		renderRow : function(_dd, _de, _df, _e0, row) {
			...
			for (var i = 0; i < _de.length; i++) {
				...
				if (col) {
					var _e3 = col.styler
							? (col.styler(row[_e2], row) || "")
							: "";
					var _e4 = col.hidden
							? "style=\"display:none;" + _e3 + "\""
							: (_e3 ? "style=\"" + _e3 + "\"" : "");
...
//							cc.push("<span class=\"tree-checkbox tree-checkbox1\"></span>");
		...
		refreshRow : function(_e5, id) {
			...
			tr.children("td").each(function() {
				...
				if (col) {
					var _ec = col.styler
							? (col.styler(row[_eb], row) || "")
							: "";
					var _ed = col.hidden ? "display:none;" + _ec : (_ec
							? _ec
							: "");
					$(this).attr("style", _ed);
					var val = null;
		...
		onBeforeRender : function(_ef, _f0, _f1) {
			...
			if (_f6) {
				...
				$.data(_ef, "treegrid").data = $.data(_ef, "treegrid").data
						.concat(_f1);
			}
			if (!_f2.remoteSort) {
				this.sort(_ef, _f1);
		...
		sort : function(_f7, _f8) {
			...
			function _fb(_fc) {
				_fc.sort(function(r1, r2) {
							return _fa(r1[_f9.sortName], r2[_f9.sortName])
									* (_f9.sortOrder == "asc" ? 1 : -1);
						});
		...
		transfer : function(_fe, _ff, data) {
			...
			while (toDo.length) {
				...
				for (var i = 0; i < rows.length; i++) {
					var row = rows[i];
					if (row.parentId == node[opts.idField]) {
						if (node.children) {
	...
	$.fn.treegrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
				...
				singleSelect : true,
				view : _c7,
				loadFilter : function(data, _101) {
					return data;
				},
				editConfig : {
					getTr : function(_102, id) {
						return $(_102).datagrid("getPanel")
				...
				eachLoadData : function(row, index) { }// zzy+ 添加加载数据的值的遍历
			});
})(jQuery);
...
$.extend($.fn.treegrid.methods, {
	...
	'add': function(jq, ops){
		// 如果当前已经有文件正在编辑，则不可用
		if(jq.treegrid('getEdittingId') != null){
			$.msg.show({
				title: '提示',
		...
		// 获取idField
		var idField = $.data(target, 'treegrid')['options']['idField'];
...
//		jq.treegrid('expand', node.id);
		...
		if(ops['isEdit']){
			...
			// 选中当前添加的记录
			jq.treegrid('select', ops['rows'][0][idField]);
			
			$.data(target, 'treegrid')['currentEdittingOldRow'] = ops['rows'][0];
			$.data(target, 'treegrid')['currentEdittingId'] = ops['rows'][0][idField];
			
			jq.treegrid('beginEdit',ops['rows'][0][idField]);
		}
	...
	'edit': function(jq){
		...
		if (node){
			$.data(target, 'treegrid')['currentEdittingOldRow'] = node;
			$.data(target, 'treegrid')['currentEdittingId'] = node[ $.data(target, 'treegrid')['options']['idField']];
			jq.treegrid('beginEdit',node.id);
		}
	},
	'save': function(jq){
		var target = null;
		...
		var id = jq.treegrid('getEdittingId');
		if(id == null){
			$.msg.show({
				title: '提示',
				msg:'没有需要保存的记录!'
		...
		var row = jq.treegrid('getCurrRow', id);
		
		$.exec($.data(target, 'treegrid')['options']['save'], row, function(data){
			$.msg.say("保存成功!");
			// 停止编辑
			jq.treegrid('endEdit', id);
			// 更新界面行记录
			jq.treegrid('updateRow', data);
			
			$.data(target, 'treegrid')['currentEdittingOldRow'] = null;
			$.data(target, 'treegrid')['currentEdittingId'] = null;
		});
	...
	'delete': function(jq){
		...
		$.msg.confirm('询问', '是否删除当前选中记录？', function(r){
			if (r){
				$.exec($.data(target, 'treegrid')['options']['remove'], node, function(data){
					$.msg.say("删除成功!");
					$.data(target, 'treegrid')['currentEdittingOldRow'] = null;
					$.data(target, 'treegrid')['currentEdittingId'] = null;
					jq.treegrid("remove", data[idField]);
				});				
			}
		});
	...
	'getEdittingId': function(jq){
		var target = null;
		jq.each(function(){target = this;});
		return $.data(target, 'treegrid')['currentEdittingId'];	
	}
});
