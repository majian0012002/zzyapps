	...
	function _15(_16) {
		...
		_18.find("div.tree-node").draggable({
			...
			onDragOver : function(e, _1a) {
				...
				if(	$(_1a).draggable("proxy")){ // zzy+
					$(_1a).draggable("proxy").removeClass("tree-dnd-no")
							.addClass("tree-dnd-yes");
				}
				$(this).removeClass("tree-node-append tree-node-top tree-node-bottom");
				if (_1b > top + (_1c - top) / 2) {
					$.msg.say("aaaaaaaaaaaa")
					if (_1c - _1b < 5) {
						$(this).addClass("tree-node-bottom");
					} else {
//						$(this).addClass("tree-node-append"); //zzy+ 不支持移动到文件夹，只支持排序
					}
				} else {
					$.msg.say("bbbbbbbbbbbb")
					if (_1b - top < 5) {
	...
	function _47(_48, ul, _49, _4a) {
		...
		function _4e(ul, _51, _52) {
			for (var i = 0; i < _51.length; i++) {
				...
				//zzy+ 
				if(!_53['text']){
					_53['text'] = _53[_4b['textField']];
				}
				
				// 遍历加载的数据
				_4b.eachLoadData.call(_48, _53, i);
...
//						});
	...
	function _56(_57, ul, _58, _59) {
		...
		_58 = _58 || {};
		//zzy 参数设置
		_5a.queryParams = _5a.queryParams || {};
		var param = $.isFunction(_5a.queryParams) ? _5a.queryParams() : _5a.queryParams;
		param = param || {};
		$.extend(_58, param);
		...
		$.query(_5a['list'], _58,  function(_5e) {
						_5e = $.decode(_5e);
						//zzy+ 转换值
						if(!$.isEmpty(_5e)){
							if(_5e['rows']){
						...
						if(_5e && _5e.length>0){
//							for (var index = 0; index < _5e.length; index++) {
//								var bean = _5e[index];
//								
//								// 遍历加载的数据
//								_5a.eachLoadData.call(_57, bean, index);
//								
//								if(!bean['iconCls']){
//									bean['iconCls'] = 'icon-tree-folder';	
//								}
//								if( $.isEmpty(bean['text']) ){
//									bean['text'] = bean[_5a['textField']];
//								}
//							}
							
							_5e = make2TreeJson(_5e);
						}
...
//				});
	...
	/**
	 * 根据parentId，生成TREE——JSON
	 * @param rows 行值
	 * @param treeJson
	 * @returns
	 */
	function make2TreeJson(rows, treeJsonArr/**private*/, deepIdx/**private*/){
		deepIdx = deepIdx ? deepIdx : 0;
		treeJsonArr = treeJsonArr ? treeJsonArr : [];
		
		if(deepIdx == 0){
			deepIdx++;
			var rowBean = getRowsByParentId(rows, null);
			return make2TreeJson(rowBean['falseRows'], rowBean['trueRows'], deepIdx);
		}else{
			$.each(treeJsonArr, function(idx, row){
				var rowBean = getRowsByParentId(rows, row['id']);
				if(rowBean['trueRows'].length > 0){
					row['children'] = make2TreeJson(rowBean['falseRows'], rowBean['trueRows'], deepIdx++);
				}
			});
			return treeJsonArr;
		}
	 ...
	 * @returns
	 */
	function getRowsByParentId(rows, parentId){
		var tmpRows = [];
		var expRows = [];
		
		$.each(rows, function(idx, row){
			if($.isEmpty(parentId) && $.isEmpty(row['parentId'], false)){
				tmpRows.push(row);
	 ...
	 * 清楚所有选中
	 */
	function clearAllChecked(_this){
		$(_this).find(".tree-checkbox1, .tree-checkbox2").each(function(){
			$(this).removeClass('tree-checkbox1 tree-checkbox2').addClass('tree-checkbox0');
//			var _11 = $(this).parent();
//			_2d(_this, _11[0], true/*!$(this).hasClass("tree-checkbox1")*/);
		});
	}	
	
	function clearChecked(_this, id){
		id = $.isString(id) ? id: id['id'];
		$(_this).find("div.tree-node[node-id=" + id + "]").find(".tree-checkbox1, .tree-checkbox2").each(function(){
			$(this).removeClass('tree-checkbox1 tree-checkbox2').addClass('tree-checkbox0');
//			var _11 = $(this).parent();
//			_2d(_this, _11[0], true/*!$(this).hasClass("tree-checkbox1")*/);
		});
	 ...
	 * @param id 可以是主键或者对象
	 */
	function setChecked(_this, id){
		id = $.isString(id) ? id: id['id'];
		$(_this).find("div.tree-node[node-id=" + id + "]").find(".tree-checkbox0, .tree-checkbox1, .tree-checkbox2").each(function(){
			$(this).removeClass('tree-checkbox0').removeClass('tree-checkbox2').addClass('tree-checkbox1');
			var _11 = $(this).parent();
			_2d(_this, _11[0], true/*!$(this).hasClass("tree-checkbox1")*/);
		});
	}
	 ...
	 * 设置选中，参数为数组
	 * @param _this 
	 * @param checkedArr 要选中的数组
	 * @returns
	 */
	function setListChecked(_this, checkedArr){
		$.each(checkedArr, function(idx, row){
			setChecked(_this, row);
		});
	}
...
//		if(_cb.url != null){
//			if(_cb.list == null){
//				_cb.list = _cb.url + '/list'
//			};
//			
//			if(_cb.remove == null){
//				_cb.remove = _cb.url + '/remove'
//			}
//			
//			if(_cb.save == null){
//				_cb.save = _cb.url + '/save'
//			}
//		}
	...
	$.fn.tree.methods = {
		...
		getAllChecked : function(jq) {
			return getAllChecked(jq[0]);
		},
		setChecked : function(jq, id) {
			return setChecked(jq[0], id);
		},
		setListChecked : function(jq, list) {
			return setListChecked(jq[0], list);
		},
		clearChecked : function(jq, id) {
			return _8a(jq[0], id);
		},
		clearAllChecked : function(jq) {
			return clearAllChecked(jq[0]);
		},
	...
	$.fn.tree.parseOptions = function(_ea) {
		...
		var cfg =  {
			...
			idField : t.attr("idField"),	//zzy+
			textField : t.attr("textField"), //zzy+
			list : t.attr("list"), //zzy+ 查询url
			save : t.attr("save"), //zzy+ 保存url
			remove : t.attr("remove") //zzy+ 删除url
		};
