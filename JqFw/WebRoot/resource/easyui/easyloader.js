...
//			css:'tree.css',
			dependencies:['datagrid', 'draggable','droppable']
		},
		panel: {
			js:'jquery.panel.js'/*,
			css:'panel.css'*/
...
//			css:'tree.css',
		...
		datebox:{
			...
			css:'datebox.css',
			dependencies:['combo', 'calendar','validatebox']
		},
		//zzy+
		datetimebox:{
			js:'jquery.datetimebox.js',
//			css:'datebox.css',
			dependencies:['datebox', 'timespinner', 'validatebox']
		},
		parser:{
			js:'jquery.parser.js'
		}
	...
	// zzy+
	modules['msg'] = modules['messager'];
	modules['grid'] = modules['datagrid'];
	...
	function loadSingle(name, callback){
		...
		if (easyloader.css && module['css']){
			if (/^http/i.test(module['css'])){
				...
				var url = easyloader.base.replace("easyui/", '') + 'themes/' + easyloader.theme + '/' + module['css'];
			}
		...
		if (/^http/i.test(module['js'])){
			...
			var url = easyloader.base + (easyloader.isDebug?'src/':'plugins/')  +  module['js'];
		}
		loadJs(url, function(){
			jsStatus = 'loaded';
			if (jsStatus == 'loaded' && cssStatus == 'loaded'){
