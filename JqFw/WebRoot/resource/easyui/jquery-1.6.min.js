...
Ext.defer(function(){
    ...
    defer: function(fn, millis, obj, args, appendArgs) {
        fn = $.util.Functions.createDelegate(fn, obj, args, appendArgs);
        if (millis > 0) {
...
$.extend($,{
    ...
    /**
     * 获取bean对象里的值
     * @param {} propStr 属性名，如：$.beanVal('name')；如果有子对象，则$.beanVal('dept.id');
     */
    beanVal: function(bean, propStr){
    	var propArr = propStr;
    	// 如果是数组
    	if( !$.isArray(propArr)){
    		propArr = propArr.split('.');
    	}
    	
    	var firstObj = bean[propArr[0]];
    	
    	if($.isEmpty(firstObj))return null;
    	...
    	if(propArr.length == 0){
    		...
    		return $.beanVal(firstObj, propArr.slice(1, propArr.length));
    	}
    },
    /**
     * 生产唯一主键
	...
	createURL: function(opts){
		if(opts.url != null){
			if(opts.list == null){
				opts.list = opts.url + '/list';
			};
			
			if(opts.remove == null){
				opts.remove = opts.url + '/remove';
			}
			
			if(opts.save == null){
				opts.save = opts.url + '/save';
			}
		}
	},
	/**
	 * 状态查询参数
	 * 即：传status=1的参数到后台，确保数据全部都是有效值，当组件没有其他查询条件时，该函数比较有用
	 * 可以直接配置：qureyParams:'$.statusParam'
	 */
	 ...
	 * @param prop 需要的属性
	 */
	fetchListProp: function(rows, prop){
		var newRows = [];
	 ...
	 * @type 
	 */
	defer: $.util.Fun.defer,
	
	/**
	 * 定时器
	 * @param booleanFun 定时器执行条件函数，如果返回true，则直接执行anctionFun，否则，延时执行
	 * @param actionFun	要执行的函数
	 * @param timeLen		定时器时长
	 */
	timer:function(booleanFun, actionFun, timeLen, scope){
		if($.util.Functions.createDelegate(booleanFun, scope || this)()){
			$.util.Functions.createDelegate(actionFun, scope || this)();
		}else{
			$.defer($.timer, timeLen, this, [booleanFun, actionFun, timeLen, scope]);
//			$.timer.defer(timeLen, this, [booleanFun, actionFun, timeLen, scope])
		}
	},
	/**
	 * 红色字体
	 ...
	 * @param {} value cookie 的值（如果为空，则是获取值）
	 * @param {} days cookie的有效天数（默认为365天）
	 */
	cookie: function(name,value,days){
		if(value){ // 如果value值存在，则设置值
			days = days||365;
		    var exp  = new Date();    //new Date("December 31, 9998");
		    exp.setTime(exp.getTime() + days*24*60*60*1000);
		    document.cookie = name + "="+ escape (value) + ";expires=" + exp.toGMTString();
		}else{// 否则获取name对应的cookie值
...
/**
 * Shorthand for {@link $.JSON#encode}
 * @param {Mixed} o The variable to encode
 * @return {String} The JSON string
 * @member Ext
 * @method encode
...
$.encode = $.JSON.encode;
/**
 * Shorthand for {@link $.JSON#decode}
 * @param {String} json The JSON string
...
jQuery.extend({
	     ...
	     * 打印对象的内容
	     */
	    alert	:	function(obj, prefixStr, likePropName){
	    	if(prefixStr !== false){
	    		try{
		    		alert($.encode(obj));
		    		return;
			...
			//自定义方式，遍历
			for( var p in obj ){
//					if(!Ext.isNumber(obj[p]) && p.indexOf("target")!=-1)
	    		if(!likePropName || likePropName==null || p.indexOf(likePropName)!=-1)
	    		s += p	+	'\t\t -> \t'	+	obj[p]	+	'\n';	
	    	}
	     ...
	     * 打印所有属性
	     */
	    alertField: function(obj, prefixStr, likePropName){
	    	var s = prefixStr ? prefixStr+"\n" : "";
...
//					if(!Ext.isNumber(obj[p]) && p.indexOf("target")!=-1)
		...
		param2Str: function( a, traditional ) {
			var s = [],
			...
			if ( jQuery.isArray( a ) || ( a.jquery && !jQuery.isPlainObject( a ) ) ) {
				...
				for ( var prefix in a ) {
					buildParams2Str( prefix, a[ prefix ], traditional, add );
				}
			}
...
function buildParams2Str( prefix, obj, traditional, add ) {
...
//				if(typeof v === "object"){
//					buildParams2Str( prefix + "." + ( typeof v === "object" ? i : "" ), v, traditional, add );
//				}else if(jQuery.isArray(v)){
					buildParams2Str( prefix + "[" + (i) + "]", v, traditional, add );
//				}
			}
		});
		...
		for ( var name in obj ) {
			buildParams2Str( prefix + "." + name, obj[ name ], traditional, add );
		}
	...
	$.extend($.fn, {
			...
			$(this).each(function(){
				if(this.elements){
					for (var index = 0; index < this.elements.length; index++) {
						$(this.elements[index]).attr('disabled', 'disabled');
					}
				}else{
					$(this).find('form').each(function(){
						$(this).disable();
					})
					$(this).find('.easyui-linkbutton').each(function(){
		...
		enable: function(){
			$(this).each(function(){
				if(this.elements){
					for (var index = 0; index < this.elements.length; index++) {
						$(this.elements[index]).removeAttr('disabled');
					}
				}else{
					$(this).find('form').each(function(){
						$(this).enable();
 ...
 * @param {} url 请求的url
 * @param {} data	请求的参数
 * @param {} succFun 成功返回函数
 * @param {} errFun  失败返回函数
 */
$.query = function(url, data, succFun, errFun) {
 ...
 * @param {} errFun  失败返回函数
 */
$.exec = function(url, data, succFun, errFun){
	$.ajax({
	  url: Wide.path + url,
	  data: $.encode(data), 	// 参数要序列化  
	  dataType: 'json',  // 返回数据类型是json
	  contentType:'application/json;charset=UTF-8', // 参数格式是json 
	  type: "POST",
	  success: function(msg){
	  	succFun(msg);
	  },
